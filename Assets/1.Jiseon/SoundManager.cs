...
public class SoundManager : MonoBehaviour
{
    ...
    private void OnCutsceneStart(PlayableDirector dir)
    {
        ...
        if (pair == null)
        {
            Debug.Log($"[SoundManager] ÄÆ¾À '{cutsceneName}'¿¡ ´ëÀÀµÇ´Â À½¾Ç ¾øÀ½ ¡æ ÇöÀç BGM À¯Áö");
            return;
        }

        // ÇöÀç ¾ÀÀÇ BGMÀÌ ¼³Á¤µÇ¾î ÀÖÀ» ¶§¸¸ ÄÆ¾ÀÀ¸·Î ÀüÈ¯
        SceneMusicPair currentScenePair = FindClip(currentSceneName, bgmClips);
        if (currentScenePair == null || bgmSource.clip == null)
        {
            Debug.Log($"[SoundManager] ÇöÀç ¾À '{currentSceneName}'¿¡ BGMÀÌ ¾øÀ¸¹Ç·Î ÄÆ¾À À½¾ÇÀ¸·Î ÀüÈ¯ÇÏÁö ¾ÊÀ½");
            return;
        }
